...
_ensure_shared_on_path()
    ...
    class SmartAssetsBuilderExtension(omni.ext.IExt):
        def on_startup(self, ext_id: str):
            self._log = get_logger("tw.zin.smart_assets_builder")
            self._window = ui.Window("SmartAssetsBuilder", width=360, height=180, visible=False)
            with self._window.frame:
                with ui.VStack(spacing=8, height=0):
                    ui.Label("SmartAssetsBuilder", height=24)
